      ...
      for (var s = 0; s < allEls.length; s++) {
        ...
        if (len < 30 && el.children.length <= 1) {
          if (/^(OBJECT|ARRAY|object|array|default|Response)$/.test(fullText) ||
              /^application\/json$/i.test(fullText)) {
            el.setAttribute('data-obj-hidden', '1');
            el.style.display = 'none';
            continue;
          }
        }
        /* REQUEST BODY — may contain child spans (* and application/json), allow up to 5 children */
        if (len < 80 && el.children.length <= 5 && /REQUEST\s*BODY/i.test(fullText) && !/Field|Type|Description/i.test(fullText)) {
          el.setAttribute('data-obj-hidden', '1');
          el.style.display = 'none';
        }
      }
