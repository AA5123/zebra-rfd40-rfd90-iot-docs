...
<body>
  ...
  (function() {
    ...
    mqttSheet.replaceSync([
      ...
      'th { background: #003d6b !important; color: #fff !important; font-weight: 600 !important; font-size: 1rem !important; letter-spacing: 0.3px !important; padding: 10px 14px !important; text-align: left !important; border-color: #002d50 !important; }',
      'td { padding: 10px 14px !important; vertical-align: top !important; font-size: 1rem !important; border: 1px solid #8a94a6 !important; word-break: break-word !important; }',
      'tr:nth-child(odd) td { background: #ffffff !important; }',
      'tr:nth-child(even) td { background: #f7f9fb !important; }',

      /* ── Headings: navy accent ── */
      '.endpoint-body .summary .title, .summary .title, div.summary div.title, .only-large-font { font-size: 2.8rem !important; font-weight: 800 !important; color: #003d6b !important; letter-spacing: 0.3px !important; border-bottom: 3px solid #003d6b !important; padding-bottom: 10px !important; display: block !important; margin-bottom: 8px !important; }',
      '.endpoint-head .descr { display: none !important; }',
      '.api-content h2, .section-gap h2, .section-gap--read-mode h2 { color: #003d6b !important; font-weight: 700 !important; border-bottom: 2px solid #003d6b !important; padding-bottom: 6px !important; margin-top: 36px !important; margin-bottom: 20px !important; font-size: 1.8rem !important; }',
      '.api-content h3, .section-gap h3 { color: #003d6b !important; font-weight: 600 !important; margin-top: 28px !important; margin-bottom: 8px !important; font-size: 1.4rem !important; }',

      /* ── Code blocks ── */
      'pre { background: #f5f5f5 !important; border-left: 4px solid #003d6b !important; border-radius: 4px !important; padding: 14px 16px !important; font-size: 0.95rem !important; line-height: 1.6 !important; max-height: 400px !important; overflow: auto !important; }',
      'code { background: #f4f4f4 !important; border-radius: 3px !important; padding: 2px 6px !important; font-size: 0.95rem !important; color: #1a1a1a !important; }',
      'pre code { background: none !important; border: none !important; padding: 0 !important; font-size: 0.95rem !important; }',

      /* ── Textareas (request bodies) ── */
      'textarea { min-height: 40px !important; max-height: 200px !important; height: auto !important; resize: vertical !important; font-size: 0.95rem !important; padding: 10px !important; }',
      ...
      /* ── Description text ── */
      '.m-markdown p, .m-markdown li { font-size: 1.1rem !important; line-height: 1.7 !important; color: #1a1a1a !important; }',
      '.m-markdown strong { color: #1a1a1a !important; }',

      /* ── Schema table: widen and improve readability ── */
      ':host { --font-size-small: 1rem !important; --font-size-regular: 1.05rem !important; --font-size-mono: 0.95rem !important; }',
      ...
      '.schema-table .td { padding: 10px 14px !important; font-size: 1.05rem !important; word-break: break-word !important; min-width: 0 !important; }',
      '.schema-table .key { min-width: 140px !important; font-size: 1.05rem !important; }',
      '.schema-table .key-type { min-width: 70px !important; white-space: nowrap !important; font-size: 1.05rem !important; }',
      '.schema-table .key-descr { min-width: 0 !important; flex: 1 !important; font-size: 1.05rem !important; }',
      '.obj-descr { font-size: 1.05rem !important; }',
      '.descr-text { font-size: 1.05rem !important; }',
      'span.descr-text { font-size: 1.05rem !important; }',
      '.schema-description { font-size: 1.05rem !important; }',
      '.toolbar-btn { font-size: 0.95rem !important; }',
      '.request-body-container *, .response-body-container * { --font-size-small: 1rem !important; --font-size-regular: 1.05rem !important; }',
      ...
      '.m-markdown blockquote { background: #e8f0f8 !important; border-left: 4px solid #003d6b !important; border-radius: 4px !important; padding: 12px 16px !important; margin: 16px 0 16px !important; font-size: 1.05rem !important; }',
    ...
    function injectGroupHeaders(sr) {
      ...
      for (var i = 0; i < navTags.length; i++) {
        ...
        if (firstMap[tagText]) {
          ...
          hdr.style.cssText = 'font-size:1rem;font-weight:800;color:#003d6b;text-transform:uppercase;letter-spacing:2px;padding:' + (isFirst ? '10px' : '18px') + ' 16px 10px;' + (isFirst ? '' : 'margin-top:8px;border-top:2px solid #003d6b;');
    ...
    function addCopyToElement(root) {
        ...
        if (len < 80 && /REQUEST\s*BODY/i.test(fullText) && !/Field|Type|Description/i.test(fullText)) {
          ...
          while (sibling) {
            ...
            if (/EXAMPLE|SCHEMA/i.test(st) && !sibling.querySelector('[data-appjson-injected]')) {
              ...
              ajLabel.style.cssText = 'position:absolute;right:0;top:0;font-size:1rem;color:#333;font-weight:400;';
      ...
      for (var r = 0; r < reqResTitles.length; r++) {
        ...
        if (txt.indexOf('REQUEST') !== -1) {
          reqResTitles[r].innerHTML = '<div style="font-size:1.8rem;font-weight:700;color:#003d6b;border-bottom:2px solid #003d6b;padding-bottom:6px;margin-top:36px;margin-bottom:12px;">Command Payload</div>' +
            '<div style="font-size:1rem;color:#333;margin:4px 0 12px;"><b>Publish to:</b> <code style="background:#f4f4f4;padding:2px 6px;border-radius:3px;">' + topicPrefix + 'cmnd/{deviceSerial}</code></div>';
        } else if (txt.indexOf('RESPONSE') !== -1) {
          reqResTitles[r].innerHTML = '<div style="font-size:1.8rem;font-weight:700;color:#003d6b;border-bottom:2px solid #003d6b;padding-bottom:6px;margin-top:36px;margin-bottom:12px;">Response Payload</div>' +
            '<div style="font-size:1rem;color:#333;margin:4px 0 12px;"><b>Published on:</b> <code style="background:#f4f4f4;padding:2px 6px;border-radius:3px;">' + topicPrefix + 'resp/{deviceSerial}</code></div>';
        }
      }
      ...
      for (var a = 0; a < boldTexts.length; a++) {
        ...
        var badges = vals.map(function(v) {
          return '<span style="display:inline-block;background:#fff;border:1px solid #bbb;border-radius:3px;padding:2px 10px;margin:2px 4px 2px 0;font-family:Consolas,Monaco,monospace;font-size:0.95rem;color:#333;">' + v + '</span>';
        }).join('');
        parentDiv.innerHTML = '<span style="font-weight:600;color:#555;font-size:0.95rem;">Enum:</span> ' + badges;
      }
      ...
      for (var j = 0; j < tas.length; j++) {
        ...
        fakePre.style.cssText = 'position:relative;background:#f6f8fa;border:1px solid #e2e8f0;border-radius:6px;padding:14px 50px 14px 16px;font-family:Consolas,Monaco,monospace;font-size:0.95rem;white-space:pre-wrap;word-break:break-word;color:#333;line-height:1.6;margin:0;overflow:auto;max-height:300px;';
    ...
    function styleHeadings(root) {
      ...
      for (var t = 0; t < titles.length; t++) {
        ...
        if (bc && !titles[t].parentElement.querySelector('[data-breadcrumb]')) {
          ...
          crumb.style.cssText = 'font-size:0.85rem;color:#666;margin-bottom:16px;font-weight:400;';
