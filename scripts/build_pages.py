...
<body>
  ...
  (function() {
    ...
    /* Force single-column layout: move code samples inline below description
       ...
       Strategy: 1) Inject aggressive CSS  2) Find dark-background divs by computed
       brightness and force their parent flex container to column direction. */
    function forceSingleColumn() {
      /* --- 1. Inject CSS override (runs once) --- */
      if (!document.getElementById('sc-force-col')) {
        var s = document.createElement('style');
        s.id = 'sc-force-col';
        s.textContent = [
          '#redoc-container > div > div:nth-child(2) { width:100%!important; max-width:100%!important; }',
          '#redoc-container > div > div:nth-child(2) > div > div { flex-direction:column!important; }',
          '#redoc-container > div > div:nth-child(2) > div > div > div { width:100%!important; max-width:100%!important; min-width:0!important; flex:1 1 100%!important; }',
          '#redoc-container > div > div:nth-child(2) > div > div > div > div { width:100%!important; max-width:100%!important; min-width:0!important; flex:1 1 100%!important; }',
          '#redoc-container > div > div:nth-child(2) > div > div > div > div > div { width:100%!important; max-width:100%!important; min-width:0!important; }',
        ].join('\\n');
        document.head.appendChild(s);
      }

      /* --- 2. Find dark panels by brightness and fix them --- */
      var wrap = el.children[0];
      if (!wrap) return;
      ...
      for (var i = 0; i < allDivs.length; i++) {
        var node = allDivs[i];
        if (node.getAttribute('data-sc-col')) continue;
        ...
        /* This is a dark panel. Check it is sizeable (not a tiny badge) */
        if (node.offsetWidth < 100 || node.offsetHeight < 50) continue;
        node.setAttribute('data-sc-col', '1');
        /* Force parent to column layout */
        var parent = node.parentElement;
        if (parent && parent !== content) {
          parent.style.setProperty('flex-direction', 'column', 'important');
          parent.style.setProperty('display', 'flex', 'important');
          /* Fix all sibling widths */
          for (var j = 0; j < parent.children.length; j++) {
            parent.children[j].style.setProperty('width', '100%', 'important');
            parent.children[j].style.setProperty('max-width', '100%', 'important');
            parent.children[j].style.setProperty('min-width', '0', 'important');
            parent.children[j].style.setProperty('flex', '1 1 100%', 'important');
          }
        }
        /* Restyle dark panel to light */
        node.style.setProperty('background', '#f7f9fb', 'important');
        node.style.setProperty('color', '#1e293b', 'important');
        node.style.setProperty('padding', '16px 24px', 'important');
        ...
        /* Fix light-on-dark text */
        var textElems = node.querySelectorAll('*');
        for (var t = 0; t < textElems.length; t++) {
          var tc = window.getComputedStyle(textElems[t]).color;
          var tm = tc.match(/rgba?\\(\\s*(\\d+),\\s*(\\d+),\\s*(\\d+)/);
          if (tm) {
            var tb = (parseInt(tm[1]) * 299 + parseInt(tm[2]) * 587 + parseInt(tm[3]) * 114) / 1000;
            if (tb > 160) textElems[t].style.setProperty('color', '#1e293b', 'important');
          }
        }
      }
    }
