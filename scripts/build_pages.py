...
<body>
  ...
  (function() {
    ...
    mqttSheet.replaceSync([
      ...
      '.m-markdown h2 { margin: 16px 0 8px 0 !important; }',
      '.m-markdown h3 { margin: 12px 0 6px 0 !important; }',
      ...
      '.endpoint-body .summary .title, .summary .title, div.summary div.title { font-size: 38px !important; font-weight: 700 !important; color: #003d6b !important; letter-spacing: 0.3px !important; border-bottom: 3px solid #003d6b !important; padding-bottom: 10px !important; display: block !important; margin-bottom: 12px !important; }',
      '.endpoint-head .descr { display: none !important; }',
      '.api-content h2, .section-gap h2, .section-gap--read-mode h2 { color: #003d6b !important; border-bottom: 2px solid #003d6b !important; padding-bottom: 6px !important; margin-top: 28px !important; font-size: 29px !important; }',
      '.api-content h3, .section-gap h3 { color: #003d6b !important; margin-top: 20px !important; font-size: 24px !important; }',
    ...
    /* Inject collapsible MANAGEMENT / CONTROL / EVENTS / DATA group headers into sidebar nav */
    function injectGroupHeaders(sr) {
      ...
      /* Also remove any previously wrapped containers */
      var oldWraps = sr.querySelectorAll('[data-tag-group-wrap]');
      for (var w = 0; w < oldWraps.length; w++) {
        var parent = oldWraps[w].parentElement;
        while (oldWraps[w].firstChild) parent.insertBefore(oldWraps[w].firstChild, oldWraps[w]);
        parent.removeChild(oldWraps[w]);
      }
      var groups = [
      ...
      /* Build a tag→group lookup */
      var tagToGroup = {};
      for (var g = 0; g < groups.length; g++) {
        for (var t = 0; t < groups[g].tags.length; t++) tagToGroup[groups[g].tags[t]] = g;
      }
      var navTags = sr.querySelectorAll('.nav-bar-tag');
      if (navTags.length === 0) return;
      /* Collect DOM elements per group: each .nav-bar-tag + its siblings (.nav-bar-path) until next .nav-bar-tag */
      var groupEls = [[], [], [], []];
      for (var i = 0; i < navTags.length; i++) {
        var tagText = navTags[i].textContent.trim();
        var gIdx = tagToGroup[tagText];
        if (gIdx === undefined) continue;
        groupEls[gIdx].push(navTags[i]);
        /* Collect .nav-bar-path items that follow this tag */
        var next = navTags[i].nextElementSibling;
        while (next && !next.classList.contains('nav-bar-tag') && !next.hasAttribute('data-tag-group-header')) {
          if (next.classList.contains('nav-bar-path')) groupEls[gIdx].push(next);
          next = next.nextElementSibling;
        }
      }
      var container = navTags[0].parentElement;
      for (var g = 0; g < groups.length; g++) {
        if (groupEls[g].length === 0) continue;
        var isFirst = (g === 0);
        /* Create header */
        var hdr = document.createElement('div');
        hdr.setAttribute('data-tag-group-header', '1');
        hdr.style.cssText = 'font-size:18px;font-weight:800;color:#003d6b;text-transform:uppercase;letter-spacing:2px;padding:' + (isFirst ? '10px' : '18px') + ' 16px 10px;' + (isFirst ? '' : 'margin-top:8px;border-top:2px solid #003d6b;') + 'cursor:pointer;display:flex;align-items:center;justify-content:space-between;user-select:none;';
        var label = document.createElement('span');
        label.textContent = groups[g].name;
        var arrow = document.createElement('span');
        arrow.textContent = '\u25B2';
        arrow.style.cssText = 'font-size:12px;transition:transform 0.2s;';
        arrow.setAttribute('data-group-arrow', '1');
        hdr.appendChild(label);
        hdr.appendChild(arrow);
        /* Create wrapper for group items */
        var wrap = document.createElement('div');
        wrap.setAttribute('data-tag-group-wrap', '1');
        wrap.style.cssText = 'overflow:hidden;transition:max-height 0.3s ease;';
        for (var el = 0; el < groupEls[g].length; el++) {
          wrap.appendChild(groupEls[g][el]);
        }
        /* Toggle collapse/expand on click */
        (function(wrapEl, arrowEl) {
          var collapsed = false;
          wrapEl._collapsed = false;
          hdr.addEventListener('click', function() {
            collapsed = !collapsed;
            wrapEl._collapsed = collapsed;
            if (collapsed) {
    ...
    function addCopyToElement(root) {
      ...
      for (var r = 0; r < reqResTitles.length; r++) {
        ...
        if (txt.indexOf('REQUEST') !== -1) {
          reqResTitles[r].innerHTML = '<div style="font-size:24px;font-weight:700;color:#003d6b;margin-bottom:4px;">Command Payload (Request)</div>' +
          ...
          reqResTitles[r].innerHTML = '<div style="font-size:24px;font-weight:700;color:#003d6b;margin-bottom:4px;">Response Payload</div>' +
    ...
    function styleHeadings(root) {
      ...
      for (var t = 0; t < titles.length; t++) {
        ...
        titles[t].style.cssText = 'font-size:34px!important;font-weight:700!important;color:#003d6b!important;letter-spacing:0.3px!important;border-bottom:3px solid #003d6b!important;padding-bottom:8px!important;display:block!important;margin-bottom:12px!important;';
      }
    }
